...
export const list = asyncHandler(async (req, res) => {
  ...
  if (search.trim()) {
    ...
    filter.$or = [{ name: rx }, { admissionNo: rx }, { phone: rx }, { father: rx }];
  }

  const docs = await Student.find(filter).populate('classId', 'name code').sort('name')
    .skip((page - 1) * limit).limit(Number(limit));
  const ids = docs.map((d) => d._id);
  const rows = await Ledger.find({ student: { $in: ids } });
  const byStudent = new Map();
  rows.forEach((r) => { const k = String(r.student); byStudent.set(k, [...(byStudent.get(k) || []), r]); });

  let students = docs.map((d) => ({ ...d.toObject(), totals: totalsFor(byStudent.get(String(d._id)) || []) }));
  ...
  res.json({ ok: true, students, total: await Student.countDocuments(filter) });
});

/** GET /api/students/:id — profile + ledger + receipts */
export const get = asyncHandler(async (req, res) => {
  const student = await Student.findById(req.params.id).populate('classId', 'name code');
  if (!student) throw new ApiError(404, 'Student not found.');
  assertClassScope(req, student.classId._id);

  const ledger = await Ledger.find({ student: student._id }).sort('order dueDate');
  const receipts = await Receipt.find({ student: student._id }).sort('-date -seq');
  res.json({
    ok: true,
    student,
    ledger: ledger.map((l) => ({ ...l.toJSON(), balance: balanceOf(l) })),
    receipts,
...
export const update = asyncHandler(async (req, res) => {
  ...
  const editable = ['name', 'dob', 'gender', 'section', 'status', 'bloodGroup',
    'father', 'mother', 'phone', 'email', 'address', 'occupation', 'aadhaarLast4', 'photo'];
